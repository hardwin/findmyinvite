...
Routes: `/form?draft=ID` and `/editor?draft=ID`. New catalogue selections include template/type/new=1 and then replace the URL with the created draft ID. Draft credentials stay in this browser; URLs alone do not grant edit access. Previous drafts remain in the design picker. Published management pages link back to both modes.
