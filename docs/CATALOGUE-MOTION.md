...
Initial five-minute runs kept players at four playing plus two prepared and heap around 0.9 MB, but browser working set rose to roughly 906 MB. The corrected fixture, with properly sized pooled videos and two pause/restart cycles, also fails: 723 MB at 120 seconds to 852 MB at 300 seconds (decimal MB), while JS heap stays below 1 MB. Disposal removes all attached sources. Native memory reclamation still needs investigation; source cleanup does not justify claiming the memory gate passed. Raw reports are in `docs/catalogue-verification/`.
