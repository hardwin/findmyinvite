...
export function walkthroughWorkerBootCommand(){
 return [
  ...
  '  heartbeat \'{"status":"running","percent":5,"label":"npm ci…","detail":"installing deps"}\'',
  '  npm ci --omit=dev',
  'fi',
  'heartbeat \'{"status":"running","percent":12,"label":"Playwright…","detail":"install chromium"}\'',
  ...
  'NODE',
  'fi',
  'heartbeat \'{"status":"running","percent":22,"label":"Starting worker…","detail":"walkthrough-cloud-worker"}\'',
  'nohup env PATH="$HOME/bin:$PATH" node scripts/walkthrough-cloud-worker.mjs > /tmp/walkthrough-worker.log 2>&1 &',
  'WORKER_PID=$!',
