...
import {galleryTierFor,templates} from '../data';
...
export default function TempleDemo(){
 ...
 const data={...defaultInvite,template,groom:template==='luxury-pink'?'Ashok':'Rohan',bride:template==='luxury-pink'?'Supriya':'Ananya',groomDetails:'With his loving family',brideDetails:'With her loving family',date:'2026-12-20',welcome:'With our families, we invite you to celebrate our wedding.',music:template==='royal-temple'?'/assets/temple/invite-bg.mp3':template==='royal-heritage-wedding'?'':'/assets/track1.mp3'};
 ...
 const tier=galleryTierFor(template,templates.find(t=>t.id===template)?.royal===true);
 return <div style={{height:'100dvh'}}>{error?<p role="alert">{error}</p>:html?<StudioPreview html={html} data={data}/>:<p>Opening your invitation…</p>}<a href={'/templates?collection='+tier+'&design='+template} style={{position:'fixed',top:16,left:16,zIndex:50,background:'white',padding:'10px 16px',borderRadius:20,color:'#244331'}}>Choose this design →</a></div>
}
