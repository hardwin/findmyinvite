...
export default function TempleDemo(){
 ...
 if(template==='royal-sanctuary'){Object.assign(data,sanctuaryDefaults)}else{data.timeline=data.timeline.map(e=>({...e,time:data.date+'T'+data.time}));data.preEvents=data.preEvents.map(e=>({...e,time:data.date+'T'+data.time}));}
