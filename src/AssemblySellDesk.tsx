 ...
 if(name==='propose_storyboard'||name==='lock_storyboard'||name==='craft_storyboard_sheet'){
  ...
  if(sb&&typeof sb==='object'){
   next.storyboard={
    ...
    locked:(name==='lock_storyboard'||Boolean(output.locked))&&sb.direction==='fpv-rear-only-v4'
   };
  }
...
export function StoryboardPreview({state,busy,onChip,onClose}:{state:SellDeskState;busy:boolean;onChip:(text:string)=>void;onClose:()=>void}){
 ...
 return <aside className="asm-sell-theme asm-story-preview" aria-label="Storyboard preview">
  ...
  <div className="asm-story-preview-body">
   ...
   {board?.sheetUrl&&(shots.length!==5||board.direction!=='fpv-rear-only-v4')&&<button className="asm-gpt-choice-submit" disabled={busy} onClick={()=>onChip("Rebuild this as a five-scene FPV photoshoot spanning at least 1 km. Strictly no walking, steps, sliding feet, backwards flight or other people. The same bride and groom are already present at distant sites, uncovered by the camera; no pop-in, dissolve or morph. Preserve each exact final-frame outfit and identity. Scenes 3 and 4: strictly show ONLY the backs of BOTH heads and torsos; both look straight away with natural neck alignment. No profiles, head turns or face reflections. Camera stays behind until an opaque non-human wipe hides the pair before any side exposure. Groom wears matching closed-toe formal shoes, bride wedding footwear; neither barefoot. No running girl, passerby, foreground person or duplicate bride anywhere. Vary rear poses and props, not head orientation. Use varied creative poses and accessories, no default facing-each-other handholding. Repeat superfast forward travel, hard ease into ultra slow-motion portrait, then forward reacceleration. Structure: zero humans or hands throughout the automatic reveal 0–6s; Save the Date payoff 3–6s; no text 6–9s; We're getting married 9–12s. Both titles large ultra-bold levitating 3D with themed materials and colors, then a 360-degree orbit with fifteen airborne depth layers in the grandest themed finale 12–15s. Preserve my theme and couple identity. Show the revised sheet.")}>Update rear views & footwear</button>}
   ...
   {board?.sheetUrl&&!board.locked&&<button className="asm-gpt-choice-submit" disabled={busy||shots.length!==5||board.direction!=='fpv-rear-only-v4'||board.revisionPending||Boolean(old)||loadedSheet!==board.sheetUrl||failedSheet===board.sheetUrl} onClick={()=>onChip('Approve storyboard\nsheetUrl: '+board.sheetUrl+'\nCompile the approved 15-second timestamped video prompt and extract the first and final panels from this approved sheet. Preserve composition and camera angle.')}>Approve storyboard → prepare video & frames</button>}
