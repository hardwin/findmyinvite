...
export function Header({full=false}:{full?:boolean}){
 ...
 return <><nav className={full?'site-nav fixed top-0 left-0 right-0 z-50':'site-nav relative'}><div className="container mx-auto flex items-center justify-between py-3 px-4 sm:px-6"><a className="font-calligraphic text-2xl text-primary font-semibold tracking-wide shrink-0" href="/">FindMyInvite</a><div className="flex items-center gap-3 sm:gap-8"><div className="hidden md:flex items-center gap-8">{full&&<><a href="#features">Features</a><a href="#how-it-works">How It Works</a><a href="#pricing">Pricing</a></>}</div><div className="nav-account"><a className="text-sm whitespace-nowrap" href="/dashboard" onClick={myTemplates}><span className="hidden sm:inline">My Templates</span><span className="sm:hidden">Templates</span></a>{user?<button type="button" className="text-sm nav-text whitespace-nowrap" onClick={signOut}>Sign out</button>:<button type="button" className="text-sm nav-text whitespace-nowrap" onClick={()=>setAuthMode('signin')}>Sign in</button>}{full&&<Button href="/templates" className="small hidden sm:inline-flex"><span className="hidden sm:inline">Get Started</span><span className="sm:hidden">Start</span></Button>}</div></div></div></nav>{authMode&&<AuthModal mode={authMode} onClose={()=>setAuthMode(null)}/>}</>;
}
