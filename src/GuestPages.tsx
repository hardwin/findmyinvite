...
export function PublicInvitation({slug}:{slug:string}){
 ...
 if(result.invitation.studioId)return <StudioGuest slug={slug}/>;
...
export function GuestManagement({slug,Editor}:{slug:string;Editor:React.ComponentType<{initialData?:InviteData;onCloudSave?:(data:InviteData)=>Promise<void>}>}){
 ...
 return <><section className="editor-shell guest-management"><h1>Your invitation is {invite.published===false?'unpublished':'published'}</h1><p>Public guest link: <a href={'/'+slug}>{location.origin}/{slug}</a></p><p>This invitation is saved to your signed-in account. Password reset is not available yet.</p><div className="editor-actions">{isRecoveryToken(credentials.token)&&<button className="fmi-button outline" onClick={()=>{
