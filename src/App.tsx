...
export default function App(){const path=location.pathname;useEffect(()=>{if(path.startsWith('/invite/')||path.startsWith('/grand-launch')||path.startsWith('/akay')||path.startsWith('/invitations'))return;const titles:Record<string,string>={'/':'Create Invitation Webpage Online for All Events','/templates':'Invitation Templates','/create':'Personalize Your Invitation','/dashboard':'My Templates'};document.title=(titles[path]||path.slice(1).replaceAll('-',' '))+' | FindMyInvite';},[path]);return path.startsWith('/akay')?<Suspense fallback={<div className="akay-gate"/>}><AkayAdmin/></Suspense>:(path==='/grand-launch'||path==='/grand-launch/')?<Suspense fallback={<div style={{minHeight:'100svh',background:'#031c17'}}/>}><GrandLaunch/></Suspense>:path==='/'?<Home/>:path==='/templates'?<Gallery/>:path.startsWith('/invite/')?<Invitation/>:path==='/create'?<Editor/>:['/dashboard','/login','/signup','/forgot-password'].includes(path)?<Dashboard/>:path.startsWith('/manage/')?<GuestManagement slug={path.split('/')[2]} Editor={Editor}/>:['/about','/contact','/terms','/privacy-policy','/refund-policy','/shipping-policy','/blog'].includes(path)||path.startsWith('/blog/')?<ContentPages/>:path.startsWith('/invitations')?<OccasionLanding/>:/^\/[a-z0-9][a-z0-9-]{2,47}$/.test(path)?<PublicInvitation slug={path.slice(1)}/>:<Other/>}
