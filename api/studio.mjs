...
const view=p=>({id:p.id,template:p.template_id,data:p.data,html:p.html,revision:p.revision,gitSha:p.git_sha,busy:Boolean(p.session_id),publishedSlug:p.published_slug});
...
async function checkpoint(p,data,html,message){
 ...
 const code=p.git_sha&&generic===genericCode(p.html)?{commit:p.git_sha}:await checkpointCode({studioId:p.id,templateId:p.template_id,html:generic,parentSha:p.git_sha||undefined});
...
export default async function handler(req,res){let locked=null;try{
 ...
 if(action==='read'){method(req,['GET']);return respond(res,200,view(p));}
 if(action==='history'){method(req,['GET']);return respond(res,200,{versions:await db('studio_versions?project_id=eq.'+p.id+'&select=revision,message,created_at&order=revision.desc&limit=30')});}
 method(req,['POST']);if(!['save','restore','run','poll','cancel','publish'].includes(action))throw new HttpError(404,'Unknown studio action.');p=await lock(p.id);locked={id:p.id,lock_until:p.lock_until};
 ...
 if(action==='poll'){
  if(!p.session_id)return respond(res,200,{...view(p),done:true});
  const sessionId=p.session_id;
  if(!Number.isFinite(Date.parse(p.run_started_at))||Date.now()-Date.parse(p.run_started_at)>300000){await update(p,{session_id:null,run_started_at:null,run_revision:null});await stopAndClose(sessionId);throw new HttpError(408,'This change exceeded the five-minute pilot limit. Your previous version is safe.');}
  if(p.run_revision!==p.revision){await update(p,{session_id:null,run_started_at:null,run_revision:null});await stopAndClose(sessionId);throw new HttpError(409,'Your draft changed during generation. The newer draft was preserved.');}
  let result;try{result=await readAgent(sessionId)}catch(e){await update(p,{session_id:null,run_started_at:null,run_revision:null});await closeAgent(sessionId);throw e;}
  if(!result.done)return respond(res,200,{...view(p),done:false,status:result.status});
  try{if(result.cancelled)return respond(res,200,{...view(await update(p,{session_id:null,run_started_at:null,run_revision:null})),done:true,message:'Change cancelled.'});
   if(p.run_revision!==p.revision)throw new HttpError(409,'Your draft changed during generation. The newer draft was preserved.');
   const next=await checkpoint(p,result.result.data,result.result.html,String(result.result.message||'Invitation updated.').slice(0,1000));
   return respond(res,200,{...view(next),done:true,message:String(result.result.message||'Invitation updated.').slice(0,1000),usage:result.usage});
  }catch(e){await update(p,{session_id:null,run_started_at:null,run_revision:null});throw e;}finally{await closeAgent(sessionId);}
 }
 if(action==='cancel'){const next=await update(p,{session_id:null,run_started_at:null,run_revision:null});if(p.session_id)await stopAndClose(p.session_id);return respond(res,200,view(next));}
 if(p.session_id)throw new HttpError(409,'Wait for the current change or cancel it first.');
 if(action==='save')return respond(res,200,view(await checkpoint(p,body.data,p.html,'Updated invitation details.')));
 if(action==='restore'){const target=body.target;if(!Number.isInteger(target)||target<0)throw new HttpError(400,'Choose a saved version.');const rows=await db('studio_versions?project_id=eq.'+p.id+'&revision=eq.'+target+'&select=data,html&limit=1');if(!rows[0])throw new HttpError(404,'Version not found.');return respond(res,200,view(await checkpoint(p,rows[0].data,rows[0].html,`Restored version ${target}.`)));}
 if(action==='run'){
 ...
 const session=await startAgent(p,body.message,body.section);try{p=await update(p,{session_id:session.id,run_started_at:new Date().toISOString(),run_revision:p.revision});}catch(e){await stopAndClose(session.id);throw e;}return respond(res,202,{...view(p),status:'Your invitation is being customized in a private workspace.'});}
 if(action==='publish'){
 ...
 if(!p.git_sha)p=await checkpoint(p,p.data,p.html,'Verified publication checkpoint.');
 ...
 return respond(res,200,{...view(p),publishedSlug:slug,url:'/'+slug,archivePending});
 }
