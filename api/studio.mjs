...
export default async function handler(req,res){let locked=null;
 ...
 if(action==='create'){method(req,['POST']);const body=await bodyJson(req);if(!EDITOR_TEMPLATES.includes(body.template))throw new HttpError(400,'Choose an available template.');const ownerHash=tokenHash(body.token);const data=draftData(body.data,body.template);try{await rate(req,'invitation-draft-create-burst',6,60);await rate(req,'invitation-draft-create-hour',30,3600)}catch(e){if(e.status===429){res.setHeader('Retry-After','60');throw new HttpError(429,'New invitation creation is temporarily limited on this network. You can still reopen your saved drafts. Please try again later.')}throw e;}const id=randomUUID(),html=validateTemplate(await baseline(body.template),await baseline(body.template));
