...
export default async function handler(req,res){let locked=null;try{
 ...
 if(action==='poll'){
  ...
  try{if(result.cancelled){await closeAgent(sessionId);return respond(res,200,{...view(await update(p,{session_id:null,run_started_at:null,run_revision:null,workspace_id:null,workspace_revision:null})),done:true,message:'Change cancelled.'});}
 ...
 if(action==='cancel'){const next=await update(p,{session_id:null,run_started_at:null,run_revision:null,workspace_id:null,workspace_revision:null});if(p.session_id)await stopAndClose(p.session_id);return respond(res,200,view(next));}
 if(p.session_id)throw new HttpError(409,'Wait for the current change or cancel it first.');
 if(action==='prepare'){await rate(req,'studio-prepare',60,3600);const warm=await ensureWorkspace(p);return respond(res,200,{...view(warm.project),ready:warm.ready});}
 if(action==='save'){let next=await checkpoint(p,body.data,p.html,'Updated invitation details.');if(p.workspace_id&&p.workspace_revision===p.revision)next=await update(next,{workspace_revision:next.revision});return respond(res,200,view(next));}
