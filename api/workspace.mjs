...
export default async function handler(req,res){try{
 ...
 if(action==='create'){method(req,['POST']);const b=await bodyJson(req,4000),html=await baseHtml(b.baseId);await rate(req,'workspace-create',100,3600);const id='sku-'+randomUUID();const row={id,base_id:b.baseId,name:text(b.name,100,true),description:'A new FindMyInvite design',collection:b.baseId==='emerald-noir'?'classic':'royal',thumbnail:assetList(html).find(x=>/\.(png|jpg|webp)$/.test(x))||'',html,assets:{}};const [design]=await db('workspace_designs',{method:'POST',headers:{Prefer:'return=representation'},body:row});return respond(res,201,design);}
