...
import {
 parseInspirationListQuery,
 cleanInspirationId,
 decideInspirationStatus,
 listInspirations,
 decideInspiration,
 runInspirationPulse,
 runManualInspirationPulse
} from '../server/akay-inspirations.mjs';
...
export default async function handler(req,res){
 try{
  ...
  if(action==='pulse'){
   ...
   return respond(res,200,await runInspirationPulse(forceSlot?{forceSlot}:{}));
  }
  ...
  if(action==='list'){
   method(req,['GET']);
   return respond(res,200,await listInspirations(parseInspirationListQuery(req.url)));
  }

  if(action==='run'){
   method(req,['POST']);
   await rate(req,'akay-style-pulse-manual',3,3600);
   const body=await bodyJson(req,4096).catch(()=>({}));
   const force=body.force!==false;
   const forceSlot=typeof body.slot==='string'?body.slot:undefined;
   return respond(res,200,await runManualInspirationPulse({force,forceSlot}));
  }

  if(action==='decide'){
   method(req,['POST','PATCH']);
   await rate(req,'akay-inspirations',60,3600);
   const body=await bodyJson(req,4096);
   return respond(res,200,await decideInspiration(cleanInspirationId(body.id||url.searchParams.get('id')),decideInspirationStatus(body.status)));
  }
