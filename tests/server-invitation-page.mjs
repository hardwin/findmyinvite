...
test('Vercel rewrites invitations to invitation-page HTML and never through guest-page or middleware',async()=>{
 ...
 assert.equal(destinations[1],'/api/blog-page');
 assert.equal(destinations[2],'/api/blog-page?slug=:slug');
 assert.equal(routes[1].source,'/blog');
 assert.equal(routes[2].source,'/blog/:slug');
 assert.equal(destinations[3],'/api/guest-page?slug=:slug');
 assert.equal(routes[3].source,'/:slug([a-z0-9][a-z0-9-]{2,47})');
 assert.equal(destinations[4],'/api/invitation-page');
 assert.equal(destinations[5],'/api/invitation-page?slug=:slug');
 assert.equal(routes[5].source,'/invitations/:slug');
 assert.equal(destinations[6],'/index.html');
