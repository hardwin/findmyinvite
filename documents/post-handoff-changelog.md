...
- RH11 KTM couple White (2026-09-22, **not published**): clone of **royal-heritage-9** (Velicha Poove, read-only) as `royal-heritage-11`. Skipped `-10` because Spiderverse occupies that slot on draft PR #24. Crafted opening (~15s, muted) + hero (~6s, muted) staged into `work/assembly-inbox/` — no AI video gens. Display name **KTM couple White**. Overlay type sampled from pin neon pink `#FF4EB8` / deep pink `#C2187A` on white paper `#F7F5F6`, mid-sky `4svh 13% 50svh 13%` (clear of the couple). Thin character-free plates: craft `plate-alt-1` reused as 1/2/3/5, craft `plate-shared` as plate-4 for Transport+Accommodation+Gifts; no frame on Welcome / Moments / Timeline / Dress / RSVP / footer; thin black→transparent vignette only. Preview couple Ashok & Supriya. **Music pending from Crafter** — no `music` field, no mp3, videos stay muted (`-an`), never mux. STOPPED before Publish / no `main` push / no Supabase apply.
