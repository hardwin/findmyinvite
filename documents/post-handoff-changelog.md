...
- Assemble dry-run (2026-09-21): `assemblePremium({dryRun:true})` plans clones and checks inbox paths only. `ensureFfmpeg()` runs after that return so GitHub Actions `verify` does not need ffmpeg. Real assemble still encodes with `-an`.
- RH10 Spiderverse (2026-09-21, **not published**): clone of **royal-heritage-9** (read-only) as `royal-heritage-10`. Pin `https://pin.it/7eJExk04U` (rainy NYC night, masked hero + blonde woman under one umbrella). FIRST still = closed rain-streaked high-rise window onto neon city; LAST still = pin couple eye-contact. xAI `image`+`last_frame` interpolate was content-moderated (twice); opening is xAI first-frame window-push (12s) then 1s dissolve + 3s last-frame hold. Hero = one 9:16 still (couple ~20% at bottom, empty mid-sky, thin neon filigree) as a 6s static loop with grain/flicker after last_frame also moderated and xAI credits exhausted. Separate reel mp3 `royal-heritage-10-music.mp3` (Instagram `CurhBG9AGDX`); videos muted (`-an`). Overlay type sampled from pin neon magenta `#E11D8F` / cyan `#12C4D8` (dark navy glow, not white on pale sky), mid-sky `4svh 13% 50svh 13%`. Thin character-free plates 1–5; no frame on Welcome / Moments / Timeline / Dress / RSVP / footer; Transport+Accommodation+Gifts share plate-4. Preview couple Ashok & Supriya. STOPPED before Publish / no `main` push / no Supabase apply.
